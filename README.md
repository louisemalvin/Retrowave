...
- Other than that, I removed the circle drawing since it does not fit the current theme. However, instead of drawing a straight line from a to b, the app draws a curved line between 2 points (x,y), with the middle point between them as the control point:
```kotlin
...
I restructured the app towards MVVM architectural pattern (or more like, VVM, since there's no real model implemented currently). I did it to solve a couple of problems:
...
## Animations!
If we play some audio while muted, the app doesn't look promising. To solve this, I've added a couple of visual feedbacks for the user to 'feel' the audio just by looking at the app. Try the app to find out more about it! (_Looking at you, three-colored buttons._)

Other than that, I also added some animations on how the waveform is drawn. Now it appears starting from left to right, instead of all at once.
